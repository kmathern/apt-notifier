...
EOF
    ...
    #!/bin/bash
    sed -i 's/[ ]*APT::Periodic::Unattended-Upgrade.*"0".*;/   APT::Periodic::Unattended-Upgrade "1";/' /etc/apt/apt.conf.d/15periodic-update
    exit 0
EOF
    cat << EOF > "$TMP"/disable_unattended_upgrades
    #!/bin/bash
    sed -i 's/[ ]*APT::Periodic::Unattended-Upgrade.*"1".*;/   APT::Periodic::Unattended-Upgrade "0";/' /etc/apt/apt.conf.d/15periodic-update
    exit 0
EOF
